...
{
    /// <summary>
    /// The RandomRoomsMapCreationStrategy creates a Map of the specified type by placing rooms
    /// randomly and then connecting them with cooridors
    /// </summary>
    ...
    public class RandomRoomsMapCreationStrategy<T> : IMapCreationStrategy<T> where T : IMap, new()
    {
        ...
        /// </summary>
        /// <remarks>
        /// The Map will be generated by trying to place rooms up to the maximum number specified in
        /// random locations around the Map. Each room will have a height and width between the
        /// minimum and maximum room size. If a room would be placed in such a way that it overlaps
        /// another room, it will not be placed. Once all rooms have have been placed, or thrown out
        /// because they overlap, corridors will be created between rooms in a random manner.
        /// </remarks>
        ...
        private static void MakeHorizontalTunnelTillCollision(T map, int xStart, int xEnd, int yPosition)
        {
            for (int x = Math.Min(xStart, xEnd); x <= Math.Max(xStart, xEnd); x++)
            {
                while (map.GetCell(x + 2, yPosition).IsWalkable)
                {
                    yPosition++;
                    MakeVerticalTunnel(map, yPosition - 1, yPosition, x);
                }
                map.SetCellProperties(x, yPosition, true, true);
            }
        }
